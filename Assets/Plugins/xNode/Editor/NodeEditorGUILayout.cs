...
namespace XNodeEditor {
    ...
    public static class NodeEditorGUILayout {
        ...
        public static void PropertyField(SerializedProperty property, GUIContent label, bool includeChildren = true, params GUILayoutOption[] options) {
            ...
            XNode.NodePort port = null;
            if(node != null) port = node.GetPort(property.name);
            PropertyField(property, label, port, includeChildren, options);
        }

        ///// <summary> Make a field for a serialized property. Automatically displays relevant node port. </summary>
        //public static void PropertyFieldWOPorts(SerializedProperty property, GUIContent label, bool includeChildren = true, params GUILayoutOption[] options)
        //{
        //    if (property == null) throw new NullReferenceException();
        //    //XNode.Node node = property.serializedObject.targetObject as XNode.Node;
        ...
        public static void PropertyField(SerializedProperty property, GUIContent label, XNode.NodePort port, bool includeChildren = true, params GUILayoutOption[] options)
        {
            ...
            //for (int i = 0; i < options.Length; i++) Debug.Log(options[i]);

            // If property is not a port, display a regular property field
            if (port == null) EditorGUILayout.PropertyField(property, label, includeChildren, options);
            else {
                Rect rect = new Rect();
